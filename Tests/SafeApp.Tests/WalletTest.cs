...
namespace SafeApp.Tests
{
    ...
    internal class WalletTest
    {
        ...
        public async Task InsertAndBalanceTest()
        {
            ...
            var (_, keyPair1) = await Session.KeysCreatePreloadTestCoinsAsync(keyPair_1_Balance);
            var (_, keyPair2) = await Session.KeysCreatePreloadTestCoinsAsync(keyPair_2_Balance);
        ...
        public async Task InsertAndGetTest()
        {
            ...
            var walletXorUrl = await api.WalletCreateAsync();
            var (xorUrl1, keyPair1) = await Session.KeysCreatePreloadTestCoinsAsync("123");
            var (xorUrl2, keyPair2) = await Session.KeysCreatePreloadTestCoinsAsync("321");
        ...
        public async Task TransferWithoutDefaultBalanceTest()
        {
            ...
            var (_, keyPair) = await Session.KeysCreatePreloadTestCoinsAsync("123");
        ...
        public async Task TransferFromZeroBalanceTest()
        {
            ...
            var fromWalletXorUrl = await api.WalletCreateAsync();
            var (_, keyPair1) = await Session.KeysCreatePreloadTestCoinsAsync("0.0");
            await api.WalletInsertAsync(fromWalletXorUrl, "TestBalance", setDefault: true, keyPair1.SK);

            var (toXorUrl, _) = await Session.KeysCreatePreloadTestCoinsAsync("0.5");

            AssertThrows(-301, () => api.WalletTransferAsync(fromWalletXorUrl, toXorUrl, "0.5", 0));

            var toWalletXorUrl = await api.WalletCreateAsync();
            var (xorurl2, keypair2) = await Session.KeysCreatePreloadTestCoinsAsync("0.5");
        ...
        public async Task TransferDifferentAmountsTest()
        {
            ...
            var (toXorUrl, keypair2) = await Session.KeysCreatePreloadTestCoinsAsync("0.5");
        ...
        public async Task TransferToSafeKeyTest()
        {
            ...
            var (xorUrl2, _) = await Session.KeysCreatePreloadTestCoinsAsync("10.0");
        ...
        public async Task TransferFromSafeKeyTest()
        {
            ...
            var (safekeyXorUrl1, _) = await Session.KeysCreatePreloadTestCoinsAsync("10");
            var (safekeyXorUrl2, _) = await Session.KeysCreatePreloadTestCoinsAsync("0");
        ...
        public async Task TransferFromUnownedWalletTest()
        {
            ...
            var (keyXorUrl1, keyPair1) = await Session.KeysCreatePreloadTestCoinsAsync("123.321");
            ...
            var (keyXorUrl, keyPair2) = await Session.KeysCreatePreloadTestCoinsAsync("123.321");
